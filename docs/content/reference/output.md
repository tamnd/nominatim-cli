...
describes what users get. Pick a format with `-o`, or let nominatim choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nominatim <command> -o table   # aligned columns for reading
nominatim <command> -o jsonl   # one JSON object per line, for piping
nominatim <command> -o json    # a single JSON array
nominatim <command> -o csv     # spreadsheet friendly
nominatim <command> -o tsv     # tab-separated
nominatim <command> -o url     # just the URL column
nominatim <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nominatim <command> --fields id,title,url
```
...
```bash
nominatim <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nominatim <command>            # a table, because this is a terminal
nominatim <command> | wc -l    # JSONL, because this is a pipe
```
